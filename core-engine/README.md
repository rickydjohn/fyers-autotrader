...
| `LLM_DECISION_INTERVAL_SECONDS` | `60` | How often to run the full LLM decision cycle (renamed from `SCAN_INTERVAL_SECONDS` post-WS-migration) |
| `GREEKS_POLL_INTERVAL_SECONDS` | `5` | Cadence of the Greeks REST poll for open option positions (renamed from `POSITION_WATCHER_INTERVAL_SECONDS`; price polling is now WS-driven) |
