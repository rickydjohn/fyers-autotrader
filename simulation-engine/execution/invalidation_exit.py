...
def build_cross_symbol_invalidation_levels(
    decision: str, peer_current_price: float, peer_ind_dict: dict
) -> Optional[dict]:
    """Same adverse-direction filter as build_invalidation_levels, but for
    the LEADING-indicator peer index.

    NIFTY leads BANKNIFTY in the Indian intraday market — when NIFTY reverses
    at a level, BANKNIFTY typically follows. Capturing NIFTY's VWAP / EMA-21
    (adverse to the trade direction at entry) gives a BANKNIFTY position an
    early-warning invalidation trigger that fires before BANKNIFTY's own
    levels would.

    The reverse — BANKNIFTY as a peer for NIFTY positions — is intentionally
    NOT supported by the caller (_peer_symbol returns "" for NIFTY symbols).
    BANKNIFTY is the lagging indicator; using it would just add noise.

    Only vwap and ema_21 are captured — CPR levels rarely move per-tick and
    the position's own CPR check is sufficient.

    Same fail-open semantics as build_invalidation_levels.
    """
    def _f(key: str) -> Optional[float]:
        v = peer_ind_dict.get(key)
        try:
            return float(v) if v is not None else None
        except (TypeError, ValueError):
            return None
    ...
    if decision == "SELL":
        return {
            k: v for k, v in candidates.items()
            if v is not None and v > peer_current_price
        } or None
    if decision == "BUY":
        return {
            k: v for k, v in candidates.items()
...
def check_cross_symbol_invalidation(
    pos: Position, peer_ltp: float
) -> Optional[str]:
    """Apply the same cross-back-through-level logic to the PEER index.

    For SELL: captured peer levels are above peer's entry-time price; peer
    crossing UP through any invalidates the (sympathetic) bearish thesis.
    For BUY: captured peer levels are below; peer crossing DOWN invalidates.

    Returns INVALIDATION_PEER_<LEVEL> on cross. None otherwise.
    """
    levels = pos.cross_symbol_invalidation_levels
    if not levels or peer_ltp <= 0:
        return None
    ...
    if pos.side == "SELL":
        for name, level in levels.items():
            if not level or level <= 0:
                continue
            if peer_ltp > level * (1.0 + buf):
                return f"INVALIDATION_PEER_{name.upper()}"
    else:
        for name, level in levels.items():
            if not level or level <= 0:
                continue
