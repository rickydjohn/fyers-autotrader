import { useState, useMemo } from 'react'
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts'
import type { PnLData, Trade } from '../../types'
import { safeFormat, parseDate } from '../../utils/date'

type ModeFilter = 'all' | 'simulation' | 'live'

interface Props {
  pnl: PnLData
  trades: Trade[]
}

export function PnLGraph({ pnl, trades }: Props) {
  const [modeFilter, setModeFilter] = useState<ModeFilter>('all')

  // Closed trades only, filtered by selected mode
  const filtered = useMemo(() => {
    const closed = trades.filter((t) => t.status !== 'OPEN' && t.pnl != null)
    if (modeFilter === 'all') return closed
    if (modeFilter === 'live') return closed.filter((t) => t.trading_mode === 'live')
    // simulation: include records without trading_mode (legacy pre-live entries)
    return closed.filter((t) => !t.trading_mode || t.trading_mode === 'simulation')
  }, [trades, modeFilter])

  // Derived stats from filtered trades
  const totalPnl  = useMemo(() => filtered.reduce((s, t) => s + (t.pnl ?? 0), 0), [filtered])
  const wins      = filtered.filter((t) => (t.pnl ?? 0) > 0)
  const losses    = filtered.filter((t) => (t.pnl ?? 0) <= 0)
  const winRate   = filtered.length ? wins.length / filtered.length : 0
  const avgWin    = wins.length    ? wins.reduce((s, t)   => s + (t.pnl ?? 0), 0) / wins.length   : 0
  const avgLoss   = losses.length  ? Math.abs(losses.reduce((s, t) => s + (t.pnl ?? 0), 0) / losses.length) : 0

  // Timeline built from filtered trade closes (sorted by exit time)
  const tradeTimeline = useMemo(() => {
    const sorted = [...filtered]
      .filter((t) => t.exit_time)
      .sort((a, b) => parseDate(a.exit_time!).getTime() - parseDate(b.exit_time!).getTime())
    let running = 0
    return sorted.map((t) => {
      running += t.pnl ?? 0
      return { time: safeFormat(t.exit_time!, 'HH:mm'), pnl: running }
    })
  }, [filtered])

  // For "all" with no trade closes yet, fall back to backend snapshot timeline
  const backendTimeline = pnl.timeline
    .filter((t) => t.timestamp && !isNaN(new Date(t.timestamp).getTime()))
    .map((t) => ({ time: safeFormat(t.timestamp, 'HH:mm'), pnl: t.cumulative_pnl }))

  const chartData  = tradeTimeline.length > 0 ? tradeTimeline : (modeFilter === 'all' ? backendTimeline : [])
  const displayPnl = modeFilter === 'all' && filtered.length === 0 ? pnl.total_pnl : totalPnl
  const isPositive = displayPnl >= 0

  return (
    <div className="bg-gray-900 rounded-lg p-4 border border-gray-800">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">P&L Today</h2>

          {/* Mode filter toggle */}
          <div className="flex bg-gray-800 rounded p-0.5 gap-0.5">
            {(['all', 'simulation', 'live'] as ModeFilter[]).map((m) => (
              <button
                key={m}
                onClick={() => setModeFilter(m)}
                className={`px-2.5 py-0.5 text-xs rounded font-mono transition-colors ${
                  modeFilter === m
                    ? m === 'live'
                      ? 'bg-amber-600 text-white'
                      : m === 'simulation'
                      ? 'bg-blue-600 text-white'
                      : 'bg-gray-600 text-white'
                    : 'text-gray-400 hover:text-gray-200'
                }`}
              >
                {m === 'all' ? 'ALL' : m === 'simulation' ? 'SIM' : 'LIVE'}
              </button>
            ))}
          </div>
        </div>

        <div className="text-right">
          <div className={`text-2xl font-bold ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
            {isPositive ? '+' : ''}₹{displayPnl.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
          </div>
          {modeFilter === 'all' && (
            <div className={`text-sm ${isPositive ? 'text-emerald-500' : 'text-red-500'}`}>
              {isPositive ? '+' : ''}{pnl.total_pnl_pct.toFixed(2)}%
            </div>
          )}
          {modeFilter !== 'all' && filtered.length > 0 && (
            <div className="text-xs text-gray-500">
              {modeFilter === 'live' ? 'Live trades only' : 'Simulation only'}
            </div>
          )}
        </div>
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-4 gap-3 mb-4 text-center">
        <Stat label="Win Rate"    value={`${(winRate * 100).toFixed(0)}%`} />
        <Stat label="Avg Win"     value={`₹${avgWin.toFixed(0)}`}  positive />
        <Stat label="Avg Loss"    value={`₹${avgLoss.toFixed(0)}`} negative />
        <Stat label="Trades"      value={`${filtered.length}`} />
      </div>

      {/* Chart */}
      {chartData.length > 0 ? (
        <ResponsiveContainer width="100%" height={160}>
          <AreaChart data={chartData}>
            <defs>
              <linearGradient id="pnlGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor={isPositive ? '#3fb950' : '#f85149'} stopOpacity={0.3} />
                <stop offset="95%" stopColor={isPositive ? '#3fb950' : '#f85149'} stopOpacity={0} />
              </linearGradient>
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
            <XAxis dataKey="time" tick={{ fill: '#8b949e', fontSize: 11 }} />
            <YAxis tick={{ fill: '#8b949e', fontSize: 11 }} tickFormatter={(v) => `₹${v.toFixed(0)}`} />
            <Tooltip
              contentStyle={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8 }}
              labelStyle={{ color: '#8b949e' }}
              formatter={(v: number) => [`₹${v.toFixed(2)}`, 'P&L']}
            />
            <Area
              type="monotone"
              dataKey="pnl"
              stroke={isPositive ? '#3fb950' : '#f85149'}
              fill="url(#pnlGradient)"
              strokeWidth={2}
            />
          </AreaChart>
        </ResponsiveContainer>
      ) : (
        <div className="h-[160px] flex items-center justify-center text-xs text-gray-600">
          No {modeFilter === 'all' ? '' : modeFilter + ' '}trades closed today
        </div>
      )}

      {/* Budget bar */}
      <div className="mt-4">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          <span>Capital Utilization</span>
          <span>{pnl.budget.utilization_pct.toFixed(1)}%</span>
        </div>
        <div className="h-1.5 bg-gray-800 rounded-full overflow-hidden">
          <div
            className="h-full bg-blue-500 rounded-full transition-all duration-500"
            style={{ width: `${Math.min(pnl.budget.utilization_pct, 100)}%` }}
          />
        </div>
        <div className="flex justify-between text-xs text-gray-500 mt-1">
          <span>Available: ₹{pnl.budget.cash.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
          <span>In Positions: ₹{pnl.budget.invested.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
        </div>
        <div className="mt-2 flex items-center justify-between bg-gray-800/50 rounded px-3 py-1.5">
          <span className="text-xs text-gray-400">Portfolio Value</span>
          <span className="text-sm font-semibold font-mono text-white">
            ₹{(pnl.budget.current ?? (pnl.budget.cash + pnl.budget.invested)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
        </div>
      </div>
    </div>
  )
}

function Stat({ label, value, positive, negative }: { label: string; value: string; positive?: boolean; negative?: boolean }) {
  return (
    <div className="bg-gray-800/50 rounded p-2">
      <div className="text-xs text-gray-500 mb-0.5">{label}</div>
      <div className={`text-sm font-semibold ${positive ? 'text-emerald-400' : negative ? 'text-red-400' : 'text-gray-200'}`}>
        {value}
      </div>
    </div>
  )
}
